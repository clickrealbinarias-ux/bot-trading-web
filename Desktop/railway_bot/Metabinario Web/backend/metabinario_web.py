...
Basado en copymetabinariofinalizado.py pero sin Telegram
"""
...
class FollowerDatabase:
    """Manejo de base de datos simple para seguidores - FUNCIÓN ORIGINAL"""
    ...
    def register_follower(user_id, username, email, password, balance_mode, amount):
        """Registrar o actualizar seguidor - FUNCIÓN ORIGINAL"""
...
class WebBotSession:
    """Sesión del bot web para cada usuario - BASADO EN BOT ORIGINAL"""
...
def get_or_create_session(user_id, username="", email="", role="follower"):
    """Obtener o crear sesión para usuario - BASADO EN BOT ORIGINAL"""
...
def send_web_notification(user_id, notification_type, data):
    """Enviar notificación via WebSocket - REEMPLAZA TELEGRAM"""
    try:
...
def notify_all_followers(notification_type, message):
    """Notificar a todos los seguidores - REEMPLAZA TELEGRAM"""
    try:
...
def web_login():
    ...
    try:
        ...
        # Intentar conectar con ExNova según el rol - FUNCIÓN ORIGINAL
        success = False
        ...
        if role == 'trader':
            ...
            if success:
                # Registrar en base de datos - FUNCIÓN ORIGINAL
...
def start_auto_copytrading():
    ...
    try:
        ...
        # Verificar que hay trader configurado - FUNCIÓN ORIGINAL
        ...
        # Obtener seguidores de la base de datos y agregarlos a la API - FUNCIÓN ORIGINAL
        followers = FollowerDatabase.get_followers()
        ...
        # Agregar seguidores a la API original - FUNCIÓN ORIGINAL
        for follower in followers:
        ...
        # Activar copytrading automático directamente sin pedir importe - FUNCIÓN ORIGINAL
        session_obj.current_step = "auto_copy_active"
        
        # Iniciar monitoreo de copytrading automático (cada seguidor usa su importe individual) - FUNCIÓN ORIGINAL
        ...
        if success:
            # Notificar a todos los seguidores - FUNCIÓN ORIGINAL
...
def execute_manual_copytrading():
    ...
    try:
        ...
        # Ejecutar copytrading manual - FUNCIÓN ORIGINAL
...
def get_system_status():
    ...
    return jsonify({
        'message': 'Metabinario Web API is running',
        'status': 'running',
        'timestamp': datetime.now().isoformat()
    })
