...
class OurCopyTradingAPI:
    ...
    def execute_copytrading_operation(self, amount: float, active: str, direction: str, duration: int) -> Dict:
        ...
        def execute_single_operation(operation_data, op_results, amount, active, direction, duration):
            ...
            try:
                ...
                success, result = instance.execute_operation(amount, active, direction, duration)
    ...
    def start_auto_copy_monitoring(self, trader_email, follower_instances, auto_copy_amount, context, user_id):
        ...
        def replicate_to_single_follower(follower):
            try:
                ...
                try:
                    ...
                    follower_data = db_manager.get_follower_stats(follower.email)
                    if follower_data and 'amount' in follower_data:
                        follower_amount = float(follower_data['amount'])
                        logger.info(f"💰 Usando importe configurado del seguidor: ${follower_amount}")
                    else:
                        follower_amount = 10.0  # Importe por defecto si no está configurado
                        logger.info(f"💰 Usando importe por defecto del seguidor: ${follower_amount}")
                except Exception as e:
