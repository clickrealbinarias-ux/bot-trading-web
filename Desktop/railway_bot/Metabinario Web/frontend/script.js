...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Inicializar API cuando esté disponible
    if (window.metabinarioAPI) {
        metabinarioAPI = window.metabinarioAPI;
    }
});
...
function showCopytradingAccess() {
    document.getElementById('copytradingAccessModal').style.display = 'block';
    document.body.style.overflow = 'hidden';
}
...
function selectRole(role) {
    ...
    // Mostrar formulario de login con el rol seleccionado
    setTimeout(() => {
        closeModal('copytradingAccessModal');
        showLogin();
        
        // Agregar indicador de rol en el formulario
        const loginForm = document.querySelector('#loginModal .modal-body');
        const existingIndicator = loginForm.querySelector('.role-indicator');
        ...
        roleIndicator.innerHTML = `
            <div class="role-badge">
                <i class="fas fa-${role === 'trader' ? 'chart-line' : 'users'}"></i>
                ${role === 'trader' ? 'Trader Principal' : 'Seguidor'}
            </div>
        `;
        
        // Insertar antes del formulario
        const form = loginForm.querySelector('form');
        loginForm.insertBefore(roleIndicator, form);
    }, 300);
}
...
function handleLogin(e) {
    ...
    // Simulate API call with ExNova connection
    setTimeout(() => {
        if (validateLogin(email, password)) {
            ...
            // Simulate ExNova connection based on role
            simulateExNovaConnection(selectedRole, email);
            
            closeModal('loginModal');
            showSuccessMessage(`¡Bienvenido! ${selectedRole === 'trader' ? 'Trader' : 'Seguidor'} conectado exitosamente`);
    ...
    }, 2000); // Increased timeout to simulate ExNova connection
}
...
// ==================== SIMULACIÓN DE CONEXIÓN EXNOVA ====================

function simulateExNovaConnection(role, email) {
    console.log(`🔐 Simulando conexión ExNova para ${role}: ${email}`);
    
    // Simular proceso de conexión
    setTimeout(() => {
        if (role === 'trader') {
            showNotification('success', {
                title: '✅ Trader Conectado',
                message: `Cuenta ${email} conectada exitosamente con ExNova`,
                type: 'success'
            });
            
            // Simular estado de trader listo
            updateTraderStatus('connected', email);
        } else {
            showNotification('success', {
                title: '✅ Seguidor Conectado', 
                message: `Cuenta ${email} conectada exitosamente con ExNova`,
                type: 'success'
            });
            
            // Simular estado de seguidor listo
            updateFollowerStatus('connected', email);
        }
    }, 1000);
}

function updateTraderStatus(status, email) {
    // Actualizar UI del trader
    const statusElement = document.getElementById('traderStatus');
    if (statusElement) {
        statusElement.innerHTML = `
            <div class="status-indicator">
                <div class="status-dot ${status === 'connected' ? 'connected' : 'disconnected'}"></div>
                <span>Trader: ${email} - ${status === 'connected' ? 'Conectado' : 'Desconectado'}</span>
            </div>
        `;
    }
}

function updateFollowerStatus(status, email) {
    // Actualizar UI del seguidor
    const statusElement = document.getElementById('followerStatus');
    if (statusElement) {
        statusElement.innerHTML = `
            <div class="status-indicator">
                <div class="status-dot ${status === 'connected' ? 'connected' : 'disconnected'}"></div>
                <span>Seguidor: ${email} - ${status === 'connected' ? 'Conectado' : 'Desconectado'}</span>
            </div>
        `;
    }
}
...
function updateUIForLoggedInUser() {
    ...
    if (navButtons && currentUser) {
        navButtons.innerHTML = `
            <div class="user-info">
                <span>Hola, ${currentUser.name} (${currentUser.role === 'trader' ? 'Trader' : 'Seguidor'})</span>
    ...
    if (isLoggedIn) {
        ...
        // Show appropriate connection status based on role
        if (currentUser.role === 'trader') {
            document.getElementById('traderStatus').style.display = 'block';
            document.getElementById('followerStatus').style.display = 'none';
        } else {
            document.getElementById('followerStatus').style.display = 'block';
            document.getElementById('traderStatus').style.display = 'none';
        }
...
// Function to disconnect follower
async function disconnectFollower() {
    if (!currentUser || currentUser.role !== 'follower') {
        alert('Solo los seguidores pueden desconectarse');
        return;
    }
    
    if (!confirm('¿Estás seguro de que quieres detener el bot? Esto desconectará tu cuenta del sistema de copytrading.')) {
        return;
    }
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/follower/disconnect`, {
            method: 'POST',
            headers: {
        ...
        if (data.success) {
            ...
            alert('Error deteniendo bot: ' + data.error);
        }
    } catch (error) {
        ...
        disconnectBtn.textContent = '🛑 Detener Bot';
    }
}

// Function to start follower bot
async function startFollowerBot() {
    if (!currentUser || currentUser.role !== 'follower') {
        alert('Solo los seguidores pueden encender su bot');
        return;
    }
    
    const startBtn = document.getElementById('startFollowerBtn');
    startBtn.classList.add('loading');
    startBtn.textContent = '⏳ Encendiendo...';
    
    try {
        const response = await fetch(`${API_BASE_URL}/follower/start-bot`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${authToken}`
        ...
        if (data.success) {
            alert('✅ Bot encendido exitosamente - Ahora recibirás operaciones del trader');
            updateFollowerBotUI(true);
        } else {
            alert('Error encendiendo bot: ' + data.error);
        }
    } catch (error) {
        console.error('Error encendiendo bot:', error);
        alert('Error de conexión con el servidor');
    } finally {
        startBtn.classList.remove('loading');
        startBtn.textContent = '▶️ Encender Mi Bot';
    }
}

// Function to stop follower bot
async function stopFollowerBot() {
    if (!currentUser || currentUser.role !== 'follower') {
        alert('Solo los seguidores pueden apagar su bot');
        return;
    }
    
    if (!confirm('¿Estás seguro de que quieres apagar tu bot? Dejarás de recibir operaciones del trader.')) {
        return;
    }
    
    const stopBtn = document.getElementById('stopFollowerBtn');
    stopBtn.classList.add('loading');
    stopBtn.textContent = '⏳ Apagando...';
    
    try {
        const response = await fetch(`${API_BASE_URL}/follower/stop-bot`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${authToken}`
        ...
        if (data.success) {
            alert('⏸️ Bot apagado - Ya no recibirás operaciones del trader');
            updateFollowerBotUI(false);
        } else {
            alert('Error apagando bot: ' + data.error);
        }
    } catch (error) {
        console.error('Error apagando bot:', error);
        alert('Error de conexión con el servidor');
    } finally {
        stopBtn.classList.remove('loading');
        stopBtn.textContent = '⏸️ Apagar Mi Bot';
    }
}

// Function to update follower bot UI
function updateFollowerBotUI(botStatus) {
    const startBtn = document.getElementById('startFollowerBtn');
    const stopBtn = document.getElementById('stopFollowerBtn');
...
// Function to check follower bot status
async function checkFollowerBotStatus() {
    if (!currentUser || currentUser.role !== 'follower') {
        return;
    }
    
    try {
        const response = await fetch(`${API_BASE_URL}/follower/bot-status`, {
            headers: {
                'Authorization': `Bearer ${authToken}`
            }
        });
        ...
        if (data.success) {
            updateFollowerBotUI(data.bot_status);
        }
    } catch (error) {
